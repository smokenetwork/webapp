        ...
        for (const [type/*, operation*/] of operations) {
          switch (type) {
            ...
            case 'withdraw_vesting':
              if (/Account registered by another account requires 10x account creation fee worth of Smoke Power/.test(errorStr))
                errorKey = 'Account requires 10x the account creation fee in Smoke Power (approximately 300 SP) before it can power down.'
              break;
            default:
              break;
          }
          if (state.hasIn(['TransactionError', type + '_listener'])) {
            ...
            if (/unknown key: /.test(errorKey)) {
              errorKey = "Smoke account doesn't exist.";
              errorStr = "Transaction failed: Smoke account doesn't exist.";
            }
